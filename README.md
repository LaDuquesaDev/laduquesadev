...
## Who I am?
 ...
 ```react
const WhoIAm = () => {
  const [user] = useState('Yulimar Duque de Alba');
  const [currentlyLearning] = useState('Flutter');
  const [askMeFor] = useState('React and React Native');
  const [funFact] = useState('I can't stand spicy');
  const [hobbies] = useState(['Walk with my dog', 'Dance', 'Sing', 'Play ukulele']);

  const getCity = () => 'Medellín, Colombia';

  const ambitions = () => ['Develop personal projects', 'Improve my habits', 'Focus on my health'];

 return { user, currentlyLearning, askMeFor, funFact, hobbies, getCity, ambitions };
};
